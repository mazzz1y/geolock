...
export class FlatDomainSuffixTreeBuilder {
  ...
  addStrictSuffix(domain, entryId) {
    addToMap(this.strictSuffix, domain.toLowerCase(), entryId);
  }
  ...
  finish() {
    ...
    const strictSuffixEntries = encodeMap(this.strictSuffix);
    return new FlatDomainSuffixTree(fullEntries, suffixEntries, this.plain, this.regex, strictSuffixEntries);
  }
}
...
export class FlatDomainSuffixTree {
  constructor(fullEntries, suffixEntries, plain, regex, strictSuffixEntries = []) {
    this.full = fullEntries;
    this.suffix = suffixEntries;
    this.strictSuffix = strictSuffixEntries;
    this.plain = plain;
    this.regex = regex;
    this.regexCompiled = new Array(regex.length).fill(null);
    this.plainChunks = null;
  }

  compilePlainChunks() {
    const chunks = [];
    for (let i = 0; i < this.plain.length; i += PLAIN_CHUNK_SIZE) {
      const slice = this.plain.slice(i, i + PLAIN_CHUNK_SIZE);
      chunks.push(new RegExp(slice.map(({ needle }) => escapeRegex(needle)).join('|')));
    }
    this.plainChunks = chunks;
    return chunks;
  }
  ...
  matchesAny(host) {
    ...
    if (binarySearch(this.full, target) >= 0) return true;

    let cursor = target;
    while (cursor) {
      if (binarySearch(this.suffix, cursor) >= 0) return true;
      if (cursor !== target && binarySearch(this.strictSuffix, cursor) >= 0) return true;
    ...
    if (this.plain.length > 0) {
      const chunks = this.plainChunks ?? this.compilePlainChunks();
      for (const chunk of chunks) {
        if (chunk.test(target)) return true;
      }
    }

    for (let i = 0; i < this.regex.length; i += 1) {
      const compiled = this.compileRegexAt(i);
      if (compiled && compiled.test(target)) return true;
    }
  ...
  lookup(host) {
    ...
    while (cursor) {
      ...
      if (cursor !== target) this.lookupExact(this.strictSuffix, cursor, matches);
      const dot = cursor.indexOf('.');
      if (dot === -1) break;
      cursor = cursor.slice(dot + 1);
    }
