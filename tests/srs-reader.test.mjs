...
export const tests = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      await saveBlob('rule-set:ads', bytes2, { bodyHash: 'y'.repeat(64), sourceUrl: 'https://x/', shaVerified: false });
      ...
      assert.ok(Array.isArray(status.rule_sets));
      assert.equal(status.rule_sets[0].name, 'ads');
      assert.ok(status.rule_sets[0].entryCount > 0);
    },
  },
  {
    ...
    run: async () => {
      ...
      await saveBlob('rule-set:r1', bytes, { bodyHash: 'a'.repeat(64) });
      ...
      await saveBlob('rule-set:r1', new TextEncoder().encode('garbage'), { bodyHash: 'b'.repeat(64) });
  ...
  },
  {
    ...
    run: async () => {
      ...
      await saveBlob('rule-set:lazy', bytes, { bodyHash: 'c'.repeat(64), savedAt: 111 });
      await geo.reloadAll(['lazy']);

      assert.equal(geo.inRuleset('lazy', 'a.lazy.example', []), null);
      assert.equal(geo.rulesetLoaded('lazy'), false);
      let status = geo.status().rule_sets.find(r => r.name === 'lazy');
      ...
      status = geo.status().rule_sets.find(r => r.name === 'lazy');
  ...
  },
  {
    ...
    run: async () => {
      ...
      await saveBlob('rule-set:dd', bytes, { bodyHash: 'd'.repeat(64) });
  ...
  },
  {
    ...
    run: async () => {
      idbData.clear();
      await saveBlob('rule-set:bad', new TextEncoder().encode('garbage'), { bodyHash: 'e'.repeat(64) });
      ...
      const status = geo.status().rule_sets.find(r => r.name === 'bad');
      ...
      await saveBlob('rule-set:bad', good, { bodyHash: 'f'.repeat(64) });
  ...
  },
  {
    ...
    run: () => {
      ...
      assert.equal(matches({ type: 'rule-set', tag: 'loaded' }, { host: 'hit.example', ips: [] }, fakeGeo), true);
      assert.equal(matches({ type: 'rule-set', tag: 'loaded' }, { host: 'miss.example', ips: [parseIp('9.9.9.9')] }, fakeGeo), true);
      assert.equal(matches({ type: 'rule-set', tag: 'loaded' }, { host: 'miss.example', ips: [parseIp('8.8.8.8')] }, fakeGeo), false);
      const trace = [];
      assert.equal(matches({ type: 'rule-set', tag: 'absent' }, { host: 'x', ips: [] }, fakeGeo, trace), UNDECIDED);
      assert.equal(trace[0].hit, null);
      assert.equal(trace[0].note, 'rule-set not loaded');
      assert.equal(matches({ type: 'rule-set', tag: '' }, { host: 'x' }, fakeGeo), false);
    },
  },
  {
    ...
    run: () => {
      ...
      assert.deepEqual(base.data_sources.rule_sets, {});
      assert.equal(validateConfig(base).ok, true);

      const good = mergeWithDefaults({
        data_sources: { rule_sets: { 'my-set_1': { url: 'https://x.example/a.srs' } } },
      });
      assert.equal(validateConfig(good).ok, true);
      assert.equal(good.data_sources.rule_sets['my-set_1'].auto_update, true);
      assert.equal(good.data_sources.rule_sets['my-set_1'].interval_hours, 24);

      const badName = mergeWithDefaults({ data_sources: { rule_sets: { '.bad': { url: '' } } } });
      assert.equal(validateConfig(badName).ok, false);

      const badUrl = mergeWithDefaults({ data_sources: { rule_sets: { ok: { url: 'http://insecure/' } } } });
      assert.equal(validateConfig(badUrl).ok, false);

      const tooMany = mergeWithDefaults({
        data_sources: { rule_sets: Object.fromEntries(Array.from({ length: 65 }, (_, i) => [`n${i}`, { url: '' }])) },
      });
  ...
  },
  {
    ...
    run: () => {
      const withMatcher = tag => mergeWithDefaults({
        rules: [{ source: { type: 'any' }, destination: { type: 'rule-set', tag } }],
      });
  ...
  },
  {
    ...
    run: async () => {
      ...
      config.data_sources.rule_sets = { ads: { url: 'https://rs.example/ads.srs', sha256_url: '', auto_update: true, interval_hours: 24 } };
      ...
      assert.ok(idbData.get('rule-set:ads'));
      assert.equal(geo.inRuleset('ads', 'a.upd.example', []), true);
      assert.equal(updater.getLastError('rule-set:ads'), null);
      ...
      assert.equal(geo.inRuleset('ads', 'a.upd.example', []), true);
      assert.match(updater.getLastError('rule-set:ads'), /parse failed/);
  ...
  },
  {
    ...
    run: async () => {
      ...
      localStore.clear();
      const config = await loadConfig();
      config.data_sources.rule_sets = { fresh: { url: 'https://rs.example/f.srs', sha256_url: '', auto_update: true, interval_hours: 24 } };
      await saveConfig(config);
      idbData.set('rule-set:fresh:meta', { key: 'rule-set:fresh:meta', bodyHash: 'z'.repeat(64), savedAt: Date.now() });
      assert.deepEqual(await updater.updateIfStale('rule-set:fresh'), { skipped: 'fresh' });

      config.data_sources.rule_sets.fresh.auto_update = false;
      await saveConfig(config);
      assert.deepEqual(await updater.updateIfStale('rule-set:fresh'), { skipped: 'disabled' });

      config.data_sources.rule_sets.fresh.auto_update = true;
      await saveConfig(config);
      idbData.delete('rule-set:fresh:meta');
      ...
      await withFetch(async () => ({ ok: true, status: 200, arrayBuffer: async () => bigger.buffer.slice(0) }),
        async () => {
          const result = await updater.updateIfStale('rule-set:fresh');
          assert.equal(result.unchanged, false);
        });
    },
  },
